...
# This script have been converted into a cookie cutter generator for OpenSCAD
# The Inkscape objects should be converted onto 3D cookie cutters ready to print .
# Same advice as above, everything doesn't work, cleaners draw make cleaner cookies.
# Have fun using this script.

# Written by Daniel C. Newman ( dan dot newman at mtbaldy dot us )
# 10 June 2012

# Converted into a Cookie Cutter Generator by Leo Marius ( contact at leomarius dot com )
# 21 June 2015
...
class OpenSCAD( inkex.Effect ):
	...
	def __init__( self ):
		...
		self.OptionParser.add_option('--height', dest='height',
			type='float', default=float( 15 ), action='store',
			help='Height (mm)' )
			
		self.OptionParser.add_option('--thickness', dest='thickness',
			type='float', default=float( 1.5 ), action='store',
			help='thickness (mm)' )	
			
		self.OptionParser.add_option('--Flanges_type', dest='Flanges_type',
			type='int', default=float( 1 ), action='store',
			help='define the flange type' )
			
		self.OptionParser.add_option('--Flange_height', dest='Flange_height',
			type='float', default=float( 1.0 ), action='store',
			help='height of the flange' )			
			
		self.OptionParser.add_option('--Flange_width', dest='Flange_width',
			type='float', default=float( 2.0 ), action='store',
			help='Width of the flange' )	
			
		self.OptionParser.add_option('--filling', dest='filling',	
			type='inkbool',	 default=False, action='store',
			help='Solid or hollow')		
		...
		self.OptionParser.add_option('--grid', dest='grid',	
			type='inkbool',	 default=False, action='store',
			help='grid or not')		

		self.OptionParser.add_option('--gridBThick', dest='gridBThick',
			type='float', default=float( 1.2 ), action='store',
			help='Grid bars thickness' )
			
		self.OptionParser.add_option('--gridBSpace', dest='gridBSpace',
			type='float', default=float( 8 ), action='store',
			help='Grid bars spacing' )	
			
		self.OptionParser.add_option('--rotateZ', dest='rotateZ',
			type='float', default=float( 45 ), action='store',
			help='Grid rotation along Z' )				
